...
library BuyScoreTokensLibrary {
    function _createBuyScoreTokensOrder(address ekoStableAddress, uint givingAmount, address stAddress, uint requestingAmount) internal returns(uint orderId){
        dsto.ExchangeInfoStorage storage es = dsto.getExchangeStorage();
        if(!EnumerableSet.contains(es.acceptedEkoStables, ekoStableAddress)){
        ...
        dsto.order memory tempOrder = dsto.order(ekoStableAddress, givingAmount, stAddress, requestingAmount, tempId, dsto.orderType.Buy, msg.sender);
    ...
    function _sellScoreTokenToABuyOrder(uint orderId) internal {
        ...
        address scoreTokenAddress = tempOrder.requestingToken;
