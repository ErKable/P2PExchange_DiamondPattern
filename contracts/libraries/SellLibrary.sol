...
library SellScoreTokenLibrary {
    ...
    function _createSellScoreTokenOrder(address stAddress, uint stAmount, address requestingEkostable, uint ekoStableAmount) internal returns(uint orderId){
        dsto.ExchangeInfoStorage storage es = dsto.getExchangeStorage();
        //address scoreTokenAddress = es.scoreTokenAddress;
        if(!EnumerableSet.contains(es.acceptedScoreTokens, stAddress)){
            revert ee.UnacceptedScoreToken(stAddress);
        }
        if(!EnumerableSet.contains(es.acceptedEkoStables, requestingEkostable)){
            revert ee.UnacceptedEkoStable(requestingEkostable);
        }
        if(IERC20(stAddress).balanceOf(msg.sender) < stAmount){
            revert ee.NotEnougthBalance(stAddress);
        }
        uint tempId = es.actualId;
        dsto.order memory tempOrder = dsto.order(stAddress, stAmount, requestingEkostable, ekoStableAmount, tempId, dsto.orderType.Sell, msg.sender);
        es.idToOrder[tempId] = tempOrder;
        EnumerableSet.add(es.addressToIds[msg.sender], tempId);
        EnumerableSet.add(es.sellIds, tempId);

        bool success = IERC20(stAddress).transferFrom(msg.sender, address(this), stAmount);
        if(!success){
