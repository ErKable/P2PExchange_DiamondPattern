...
library ViewsLibrary {
   ...
   /*  function _getScoreTokenAddress() internal view returns(address scoreTokenAddress){
    ...
    function _getAcceptedScoreToken(address scoreTokenAddress) internal view returns(bool isAccepted){
        dsto.ExchangeInfoStorage storage es = dsto.getExchangeStorage();
        isAccepted = EnumerableSet.contains(es.acceptedScoreTokens, scoreTokenAddress);
    }

    function _getAcceptedEkoStable(address ekoStableAddress) internal view returns(bool isAccepted){
        dsto.ExchangeInfoStorage storage es = dsto.getExchangeStorage();
        isAccepted = EnumerableSet.contains(es.acceptedEkoStables, ekoStableAddress);
    }
    ...
    function _getAcceptedScoreTokens() internal view returns(address[] memory acceptedST){
        dsto.ExchangeInfoStorage storage es = dsto.getExchangeStorage();
        uint length = EnumerableSet.length(es.acceptedScoreTokens);
        acceptedST = new address[](length);
        for(uint i = 0; i < length; ++i){
            acceptedST[i] = EnumerableSet.at(es.acceptedScoreTokens, i);
        }
    }

    function _getAcceptedEkoStables() internal view returns(address[] memory acceptedES){
        dsto.ExchangeInfoStorage storage es = dsto.getExchangeStorage();
        uint length = EnumerableSet.length(es.acceptedEkoStables);
        acceptedES = new address[](length);
        for(uint i = 0; i < length; ++i){
            acceptedES[i] = EnumerableSet.at(es.acceptedEkoStables, i);
        }
    }
