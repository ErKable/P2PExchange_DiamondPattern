...
contract ViewsFacet {
    ...
    function getAcceptedScoreToken(address scoreTokenAddress) external view returns(bool isAccepted){
        return vl._getAcceptedScoreToken(scoreTokenAddress);
    }
    ...
    function getAcceptedScoreTokens() external view returns(address[] memory acceptedST){
        acceptedST = vl._getAcceptedScoreTokens();
    }

    function getAcceptedEkoStables() external view returns(address[] memory acceptedES){
        acceptedES = vl._getAcceptedEkoStables();
    }
