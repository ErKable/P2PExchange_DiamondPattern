...
describe(`Simple Diamond Contract Test`, function(){
  ...
  /*   it(`Should deploy storage library`, async function(){
    ...
    it(`Should add the scoreToken to the accepted tokens`, async function(){
        const ownerActionFacet = await ethers.getContractAt('OwnerActionFacet', diamondAddress, owner)
        await ownerActionFacet.addScoreToken(tokenAddress)

        const viewFacet = await ethers.getContractAt('ViewsFacet', diamondAddress, owner)
        let isAccepted = await viewFacet.getAcceptedScoreToken(tokenAddress)
        assert.equal(isAccepted, true)
    })
   /*  it(`Should set the scoreToken address`, async function(){
    ...
    it(`Should create a buy order with userOne`, async function(){
        ...
        console.log(buyId) */
        await buyFacet.createBuyScoreTokensOrder(ekoStableAddress, ekoStableAmount, tokenAddress, scoreAmount)
    ...
    it(`Should create a sell score token order with user one`, async function(){
        ...
        await sellFacet.createSellScoreTokenOrder(tokenAddress, scoreAmount, ekoStableAddress, ekoStableAmount)
    ...
    it(`Should call the getScoreTokens function`, async function(){
        const viewFacet = await ethers.getContractAt('ViewsFacet', diamondAddress, owner)
        let acceptedST = await viewFacet.getAcceptedScoreTokens()
        console.log(`Accepted score token: ${acceptedST}`)
    })
    it(`Should call the getEkoStables function`, async function(){
        const viewFacet = await ethers.getContractAt('ViewsFacet', diamondAddress, owner)
        let acceptedES = await viewFacet.getAcceptedEkoStables()
        console.log(`Accepted eko stables: ${acceptedES}`)
    })
    it(`should remove the ekoStable and scoreToken addresses`, async function(){
        const ownerActionFacet = await ethers.getContractAt('OwnerActionFacet', diamondAddress, owner)

        await ownerActionFacet.removeEkostable(ekoStableAddress)
        await ownerActionFacet.removeScoreToken(tokenAddress)

        const viewFacet = await ethers.getContractAt('ViewsFacet', diamondAddress, owner)
        let stAccepted = await viewFacet.getAcceptedScoreToken(tokenAddress)
        let esAccepted = await viewFacet.getAcceptedEkoStable(ekoStableAddress)

        assert.equal(stAccepted, false)
        assert.equal(esAccepted, false)
    })
    it(`Should deploye new STs and ESs`, async function(){
    ...
    it(`Should add the new STs and ESs to the accepted payments`, async function(){
        const ownerActionFacet = await ethers.getContractAt('OwnerActionFacet', diamondAddress, owner)
        ...
        assert.sameDeepMembers([ccSTAddress, sdSTaddress, cmSTaddress], acceptedST)
        assert.sameDeepMembers([ekoBUSDaddress, ekoUSDTaddress, ekoDAIaddress], acceptedES)
    })
    it(`Should fund userone and usertwo with the new tokens`, async function(){
        await ekoBUSD.connect(owner).transfer(userOneAddress, '1000000')
        await ekoDAI.connect(owner).transfer(userOneAddress, '1000000')
        await ekoUSDT.connect(owner).transfer(userOneAddress, '1000000')

        await ccST.connect(owner).transfer(userOneAddress, '10000000000')
        await sdST.connect(owner).transfer(userOneAddress, '10000000000')
        await cmST.connect(owner).transfer(userOneAddress, '10000000000')
        ...
        await ekoBUSD.connect(owner).transfer(userTwoAddress, '1000000')
        await ekoDAI.connect(owner).transfer(userTwoAddress, '1000000')
        await ekoUSDT.connect(owner).transfer(userTwoAddress, '1000000')

        await ccST.connect(owner).transfer(userTwoAddress, '10000000000')
        await sdST.connect(owner).transfer(userTwoAddress, '10000000000')
        await cmST.connect(owner).transfer(userTwoAddress, '10000000000')
    ...
    it(`Should creater a sell order for each score token with user one`, async function(){
        var scoreAmount = '1000'
        var ekoStableAmount = '10' 
        const sellFacet = await ethers.getContractAt('SellScoreTokenFacet', diamondAddress, userOne)
        await ccST.connect(userOne).approve(diamondAddress, scoreAmount)
        await sellFacet.createSellScoreTokenOrder(ccSTAddress, scoreAmount, ekoBUSDaddress, ekoStableAmount)
        await sdST.connect(userOne).approve(diamondAddress, scoreAmount)
        await sellFacet.createSellScoreTokenOrder(sdSTaddress, scoreAmount, ekoDAIaddress, ekoStableAmount)
        await cmST.connect(userOne).approve(diamondAddress, scoreAmount)
        await sellFacet.createSellScoreTokenOrder(cmSTaddress, scoreAmount, ekoUSDTaddress, ekoStableAmount)
    ...
    it(`Should creater a buy order for each eko stable with user two`, async function(){
        var scoreAmount = '1000'
        var ekoStableAmount = '10'
        const buyFacet = await ethers.getContractAt('BuyScoreTokenFacet', diamondAddress, userTwo)
        await ekoBUSD.connect(userTwo).approve(diamondAddress, ekoStableAmount)
        await buyFacet.createBuyScoreTokensOrder(ekoBUSDaddress, ekoStableAmount, ccSTAddress, scoreAmount)

        await ekoUSDT.connect(userTwo).approve(diamondAddress, ekoStableAmount)
        await buyFacet.createBuyScoreTokensOrder(ekoUSDTaddress, ekoStableAmount, cmSTaddress, scoreAmount)

        await ekoDAI.connect(userTwo).approve(diamondAddress, ekoStableAmount)
        await buyFacet.createBuyScoreTokensOrder(ekoDAIaddress, ekoStableAmount, sdSTaddress, scoreAmount)
